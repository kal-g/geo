...
void Sphere::buildVerticesSmooth()
{
    ...
    for (int i = 0; i <= stackCount; ++i)
    {
        ...
        for (int j = 0; j <= sectorCount; ++j)
        {
            ...
            auto latLong = LatLong(stackAngle, sectorAngle);
            auto euclid = latLong.toEuclidean();
            addVertex(euclid.x, euclid.y, euclid.z);

            // normalized vertex normal
            nx = euclid.x * lengthInv;
            ny = euclid.y * lengthInv;
            nz = euclid.z * lengthInv;
            addNormal(nx, ny, nz);
...
{
    std::cout << "change " << from << " " << to << std::endl;
